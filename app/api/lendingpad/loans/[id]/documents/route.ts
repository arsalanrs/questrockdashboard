...
import { getLendingPadLoanDocuments } from "@/lib/lendingpad/client";
import { normalizeLendingPadLoanUuid } from "@/lib/lendingpad/parse-response";

/**
 * GET — document metadata from LendingPad (not file bytes).
 */
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const creationPeriod = url.searchParams.get("creationPeriod") ?? undefined;
  ...
  try {
    const documents = await getLendingPadLoanDocuments(lpUuid, creationPeriod);
    return NextResponse.json({ loanId, lendingpadLoanUuid: lpUuid, documents });
  } catch (e) {
